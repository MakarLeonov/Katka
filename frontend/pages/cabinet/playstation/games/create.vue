<template>
    <div class="wrapper">
        ...
        <VeeForm v-slot="{ validate }" class="playstation-games-create">
            <div class="playstation-games-create__input">
                ...
                <Transition>
                    <div v-show="fields.name.length" @click="fields.name = ''"
                        class="playstation-games-create__input-clear">
            ...
            <div class="playstation-games-create__input">
                ...
                <Transition>
                    <div v-show="fields.author.length" @click="fields.author = ''"
                        class="playstation-games-create__input-clear">
                        <span class="material-symbols-outlined">close</span>
                    </div>
                </Transition>
            ...
            <div class="playstation-games-create__platforms-title">Платформы:</div>
            <div class="playstation-games-create__platforms">
                <div class="playstation-games-create__platforms-agreement">
                    <VeeField id="applicationFormConfirm" v-model="confirmed" name="confirm" type="checkbox"
                        :rules="{ required: true }" />

                    <label for="applicationFormConfirm">
                        <span class="playstation-games-create__platforms-agreement-checkbox"
                            :class="{ 'is-active': confirmed }">
            ...
            <div class="playstation-games-create__grid">
                <div class="playstation-games-create__input">
                    <!-- <input v-model="fields.price" placeholder="Цена"> -->
                    <VeeField v-model.trim="fields.price" name="price" placeholder="Цена *"
                        :rules="{ required: true, max: 10 }" />
                    <VeeErrorMessage name="price" class="errorMessage" />

                    <Transition>
                        <div v-show="fields.price.length" @click="fields.price = ''"
                            class="playstation-games-create__input-clear">
                            <span class="material-symbols-outlined">close</span>
                        </div>
                    </Transition>
                ...
                <div class="playstation-games-create__input">
                    <!-- <input v-model="fields.memory" placeholder="Память"> -->
            ...
            <Transition>
                <div v-show="conditions" @click="clearFrom" class="playstation-games-create__clear-from">
                    <span class="material-symbols-outlined">close</span>
                </div>
            </Transition>
        </VeeForm>
        ...
        if (ctx.rule) {
            ...
            if (ctx.rule.name === 'max' && Array.isArray(ctx.rule.params) && typeof ctx.rule.params?.[0] === 'number') {
                return (
                    ...
                    // usePluralize(ctx.rule.params?.[0], ['символ', 'символа', 'символов'])
                );
            }
        }
...
const sendForm = async (validate: FormContext['validate']) => {
    ...
    if (valid) {
        // исключим двойную отправку формы, она отключит pointer-events, в функцию больше не попадем
        isFormSended.value = true;

        // отправляем данные на сервер
        // useStrapi()('/api/requests-from-mvps', {
        //     method: 'POST',
        //     body: {
        //         data: {
        //             type: formIsCustomer ? 'customer' : 'specialist',
        //             ...formFields.value,
        //         },
        //     },
        // })
